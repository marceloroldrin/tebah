# Curso de Introdução ao Desenvolvimento Web com IA

Bem-vindo ao repositório do curso de desenvolvimento web do **Centro Socioeducativo Tebah**! 🚀

Este curso foi criado especialmente para adolescentes do sertão baiano, com foco em aprendizado prático, projetos reais e uso inteligente de IA como ferramenta de estudo.

---

## 🎯 Sobre o Curso

**Duração:** 6 meses  
**Encontros:** 1-2 vezes por semana, 2h cada  
**Formato:** Presencial, prático, baseado em projetos  
**Projeto Principal:** Cardápio digital para comércios locais

### O que você vai aprender?

- ✅ **HTML5** - Estrutura semântica e acessível
- ✅ **CSS3** - Design moderno e responsivo
- ✅ **JavaScript** - Interatividade e lógica
- ✅ **Node.js** - Introdução a backend
- ✅ **Git/GitHub** - Versionamento e colaboração
- ✅ **IA como ferramenta** - Uso crítico e inteligente

---

## 📚 Navegação Rápida

### 📖 Documentação
- **[COMECE-AQUI.md](COMECE-AQUI.md)** - Guia de primeiros passos
- **[PLANO-DO-CURSO.md](PLANO-DO-CURSO.md)** - Plano completo detalhado
- **[ESTRUTURA.md](ESTRUTURA.md)** - Organização do repositório
- **[INDICE.md](INDICE.md)** - Índice de todos os encontros
- **[TAREFAS.md](TAREFAS.md)** - Checklist de conteúdo

### 📁 Material do Curso

#### Módulo 1 - HTML
- [Semana 1: Introdução Web e HTML Semântico](modulo-1-html/semana-1/)
- [Semana 2: Acessibilidade e Formulários](modulo-1-html/semana-2/)
- [Semana 3: Planejamento do Cardápio](modulo-1-html/semana-3/)
- [Semana 4: Git/GitHub e Revisão](modulo-1-html/semana-4/)

#### Módulo 2 - CSS
- [Semana 1: Fundamentos CSS e Flexbox](modulo-2-css/semana-1/)
- [Semana 2: Grid e Responsividade](modulo-2-css/semana-2/)
- [Semana 3: BEM e Bibliotecas](modulo-2-css/semana-3/)
- [Semana 4: Animações e Apresentação](modulo-2-css/semana-4/)

---

## 🤖 Filosofia de Uso da IA

Este curso tem uma abordagem única sobre IA:

### ❌ NÃO use IA para:
- Fazer o trabalho por você
- Copiar código sem entender
- Pular o processo de aprendizado

### ✅ USE IA para:
- Revisar seu código depois de pronto
- Entender conceitos difíceis
- Debugar erros
- Expandir seu conhecimento

**Regra de ouro:** Primeiro faça manualmente, depois use IA para revisar e melhorar!

---

## 🎓 Estrutura dos Encontros

Cada encontro segue este formato:

1. **📚 Teoria** - Conceitos fundamentais
2. **💻 Prática** - Exercícios hands-on
3. **🏠 Para Casa** - Tarefas para consolidar
4. **🤖 Uso da IA** - Como usar IA neste tópico
5. **✅ Checklist** - Verificação de aprendizado

---

## 🚀 Como Começar

1. **Leia primeiro:** [COMECE-AQUI.md](COMECE-AQUI.md)
2. **Entenda a estrutura:** [ESTRUTURA.md](ESTRUTURA.md)
3. **Veja o plano completo:** [PLANO-DO-CURSO.md](PLANO-DO-CURSO.md)
4. **Comece o Módulo 1:** [modulo-1-html/semana-1/encontro-1-introducao-web-html/](modulo-1-html/semana-1/encontro-1-introducao-web-html/)

---

## 📊 Progresso do Material

### Conteúdo Disponível

- ✅ **Módulo 1 - HTML** (8 encontros completos)
- ✅ **Módulo 2 - CSS** (8 encontros completos)
- ⏳ **Módulo 3 - JavaScript** (em desenvolvimento)
- ⏳ **Módulo 4 - JavaScript Avançado** (em desenvolvimento)
- ⏳ **Módulo 5 - Node.js** (em desenvolvimento)
- ⏳ **Módulo 6 - Projeto Final** (em desenvolvimento)

### Estatísticas
- 📝 **16 encontros** documentados
- 💻 **13+ exemplos** de código
- 🎯 **10+ desafios** práticos
- 📚 **5 guias** de documentação

---

## 🎯 Projeto Âncora: Cardápio Digital

Durante todo o curso, você vai construir um **cardápio digital completo** para comércios locais:

- **Módulo 1:** Estrutura HTML semântica
- **Módulo 2:** Design responsivo com CSS
- **Módulo 3:** Interatividade com JavaScript
- **Módulo 4:** Integração com APIs
- **Módulo 5:** Backend com Node.js
- **Módulo 6:** Deploy e apresentação

---

## 💡 Dicas para Aproveitar o Curso

1. **Pratique todos os dias** - Mesmo que seja 30 minutos
2. **Faça todos os exercícios** - Não pule nenhum
3. **Use o Git desde o início** - Versione tudo
4. **Peça ajuda quando travar** - Comunidade e IA estão aqui
5. **Construa seu portfólio** - Documente seu progresso
6. **Conecte-se com comércios locais** - Projetos reais são valiosos

---

## 🤝 Contribuindo

Este material está em constante evolução. Se você encontrar erros, tiver sugestões ou quiser contribuir:

1. Abra uma issue
2. Faça um fork
3. Envie um pull request

---

## 📞 Contato

**Centro Socioeducativo Tebah**  
Foco: Adolescentes do sertão baiano

---

## 📄 Licença

Este material é de uso educacional livre.

---

**Pronto para começar?** 🚀  
👉 [Clique aqui para começar!](COMECE-AQUI.md)
